...
def load_chembookv2(view_data_dir, n_splits=5, idx_split=0, test_size=0.2, seed=1024):
    ...
    # xx_train = []
    # xx_test = []
    # for i in [1, 4, 9, 3, 2, 8, 5, 7]:
    #     xx_train.append(view_train_x[i])
    #     xx_test.append(view_test_x[i])

    x_train = np.load(os.path.join('/export/data/', 'output_data_train.npy'))
    x_train_y = np.load(os.path.join('/export/data/', 'output_data_train_y.npy'))

    x_test = np.load(os.path.join('/export/data', 'output_data_test.npy'))
    x_test_y = np.load(os.path.join('/export/data', 'output_data_test_y.npy'))

    view_train_x.append(x_train)
    view_test_x.append(x_test)


    # return xx_train, train_y, xx_test, test_y
...
class Multi_view_data(Dataset):
    ...
    def __init__(self, root, train=True, idx_split=0):
        ...
        view_data_dir = os.path.join('/export/lxy/TEC/ChemBookv2/view')

        view_train_x, train_y, view_test_x, test_y = load_chembookv2(view_data_dir, n_splits=5, idx_split=self.idx_split, test_size=0.2, seed=1024)
