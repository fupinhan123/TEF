...
def load_Reuter(view_data_dir, n_splits=5, idx_split=0, test_size=0.2, seed=1024):
    print('********************** idx_split:', idx_split)
    view_names = ['EN', 'FR', 'GR', 'IT', 'SP']
    ...
    x_train = np.load(os.path.join('/export/fupinhan/', 'output_data_train.npy'))
    x_train_y = np.load(os.path.join('/export/fupinhan/', 'output_data_train_y.npy'))

    x_test = np.load(os.path.join('/export/fupinhan/', 'output_data_test.npy'))
    x_test_y = np.load(os.path.join('/export/fupinhan/', 'output_data_test_y.npy'))
...
class Multi_view_data(Dataset):
    ...
    def __init__(self, root, train=True, idx_split=0):
        ...
        view_data_dir = os.path.join('/data/lxy/data_lxy/Reuters3noisy/')

        view_train_x, train_y, view_test_x, test_y = load_Reuter(view_data_dir, n_splits=5, idx_split=self.idx_split, test_size=0.2, seed=1024)
