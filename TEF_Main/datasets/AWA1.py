...
def load_AWA1(view_data_dir, n_splits=5, idx_split=0, test_size=0.2, seed=1024):
    print('********************** idx_split:', idx_split)
    view_names = ['cq-hist', 'lss-hist', 'phog-hist', 'sift-hist',
                  'surf-hist', 'rgsift-hist', 'res101']
    ...
    x_train = np.load(os.path.join('data/',
                                   'output_data_train4.npy'))

    x_train_y = np.load(os.path.join('data/',
                                     'output_data_train.npy'))

    x_test = np.load(os.path.join('data/',
                                  'output_data_test.npy'))
    x_test_y = np.load(os.path.join('data/',
...
class Multi_view_data(Dataset):
    ...
    def __init__(self, root, train=True, idx_split=0):
        ...
        view_data_dir = os.path.join('/export/lxy/TEC/AWA1/view')

        view_train_x, train_y, view_test_x, test_y = load_AWA1(view_data_dir, idx_split=self.idx_split, test_size=0.2, seed=1024)
