...
def load_audio(view_data_dir, n_splits=5, idx_split=0, test_size=0.2, seed=1024):
    print('********************** idx_split:', idx_split)
    view_names = ['wav_ecapa', 'wav_resnet', 'wav_fbank', 'wav_mfcc', 'wav_spec']
    ...
    for view_name in view_names:
        ...
        print('{}:{}'.format(view_name, x.shape))
...
class Multi_view_data(Dataset):
    ...
    def __init__(self, root, train=True, idx_split=0):
        ...
        view_data_dir = os.path.join('/export/lxy/TEC/audio/view')

        view_train_x, train_y, view_test_x, test_y = load_audio(view_data_dir, n_splits=5, idx_split=self.idx_split, test_size=0.2, seed=1024)
